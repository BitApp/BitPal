...
const svgs = {
  ...
  ethDefaultBig:`
    ...
    <g id="ETH" style="transform:scale(0.7) translate(9px,8px);width:40px">
    <g transform="translate(13.000000, 10.000000)">
        <linearGradient id="Triangle-2_1_" gradientUnits="userSpaceOnUse" x1="-12.3304" y1="11.4739" x2="-12.3304" y2="12.3991" gradientTransform="matrix(13.6047 0 0 -8.063 164.4451 96.2901)">
        <stop  offset="0" style="stop-color:#490DB1"/>
        <stop  offset="1" style="stop-color:#2A1860"/>
  ...
  bchDefault:`<path fill="#fff" style="transform:scale(0.7) translate(9px,9px);width:40px" d="M7.891,2.66L6.627,0l-1.65,0.826l1.239,2.608c-0.431,0.222-0.867,0.457-1.3,0.691L3.661,1.484L2.014,2.307l1.273,2.612
	C3.13,5.153,3.025,5.225,2.8,5.361C2.65,5.451,1.717,5.906,0,6.727l0.874,1.731c0,0,1.199-0.646,1.19-0.613
	c0.664-0.342,1.072-0.072,1.301,0.225l1.53,3.034c-0.071-0.148,0.646,1.271,2.149,4.263c0.075,0.221,0.118,0.614-0.345,0.854
	c0.03,0.008-1.192,0.613-1.192,0.613l0.65,2.104l2.134-1.099c0.397-0.203,0.624-0.324,1.009-0.52L10.609,20l1.683-0.82l-1.306-2.677
	c0.455-0.222,0.898-0.426,1.323-0.645l1.291,2.646l1.631-0.796l-1.294-2.653c2.677-1.575,4.381-3.423,3.329-6.105
	c-0.846-2.161-2.289-2.575-4.041-2.142c0.709-0.968,0.878-2.117,0.03-3.45C12.065,1.519,10.1,1.754,7.891,2.66L7.891,2.66z
	 M13.903,10.28c1.016,2.013-2.586,3.578-3.696,4.151l-1.799-3.569C9.519,10.29,12.843,8.181,13.903,10.28z M10.601,5.638
  c0.923,1.832-2.092,3.114-3.017,3.59L5.951,5.992C6.876,5.516,9.637,3.728,10.601,5.638z"/>`,
  bchDefaultBig:`<path fill="#fff" style="transform:scale(1.1) translate(12px,11px);width:40px" d="M7.891,2.66L6.627,0l-1.65,0.826l1.239,2.608c-0.431,0.222-0.867,0.457-1.3,0.691L3.661,1.484L2.014,2.307l1.273,2.612
	C3.13,5.153,3.025,5.225,2.8,5.361C2.65,5.451,1.717,5.906,0,6.727l0.874,1.731c0,0,1.199-0.646,1.19-0.613
	c0.664-0.342,1.072-0.072,1.301,0.225l1.53,3.034c-0.071-0.148,0.646,1.271,2.149,4.263c0.075,0.221,0.118,0.614-0.345,0.854
	c0.03,0.008-1.192,0.613-1.192,0.613l0.65,2.104l2.134-1.099c0.397-0.203,0.624-0.324,1.009-0.52L10.609,20l1.683-0.82l-1.306-2.677
	c0.455-0.222,0.898-0.426,1.323-0.645l1.291,2.646l1.631-0.796l-1.294-2.653c2.677-1.575,4.381-3.423,3.329-6.105
...
module.exports = {
  create: async function(ctx, next) {
    ...
    if (/^github\.flat$/i.test(style)) {
    ...
    } else if(/^default\.classic\.big$/i.test(style)) {
      ...
      text.move(40 + shadowOffset + attrs.textDefaultBig.paddingLeft, attrs.textDefaultBig.paddingTop + shadowOffset)
      let textBox = text.bbox()
      ...
      link.svg(svgs[symbol+'DefaultBig'])
      ...
      ctx.type = responseType
      ctx.body = draw.svg()
    } else if(/^default\.flat\.big$/i.test(style)) {
      const shadowOffset = 4
      // 默认按钮样式
      let text = link.text(str).fill('#333')
      text.font({
      ...
      text.move(20 + shadowOffset + attrs.textDefault.paddingLeft, attrs.textDefault.paddingTop + shadowOffset)
      let textBox = text.bbox()
      let iconRect = link.rect(20, 20).fill(attrs.bg[symbol]).move(shadowOffset, shadowOffset)
      let rightRect = link.rect(textBox.width + 23, 20).fill(color || 'white').move(20 + shadowOffset, shadowOffset)
      link.svg(svgs[symbol+'Default'])

      let showdowRect = draw.rect(iconRect.bbox().width + rightRect.bbox().width, 20).move(shadowOffset, shadowOffset).fill('#333333').opacity(0.2)
      showdowRect.filter(add=>{
       add.offset(0, 1).in(add.sourceAlpha).gaussianBlur(2)
      })
      ...
      draw.width(iconRect.bbox().width + rightRect.bbox().width + shadowOffset * 2).height(20 + shadowOffset * 2)
      var ellipse = draw.rect(iconRect.bbox().width + rightRect.bbox().width, 20).radius(2).move(shadowOffset, shadowOffset)

      var clip = draw.clip().add(ellipse)
      link.clipWith(clip)

      var shadowBox = draw.rect(iconRect.bbox().width + rightRect.bbox().width + shadowOffset * 2, 20 + shadowOffset * 2).radius(2)
      showdowRect.clipWith(draw.clip().add(shadowBox))
